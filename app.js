  ...
  morganLogger ('dev', {
    stream: log.logger.stream ({
      ...
      filter: function (message) {
        ...
        return !(message.includes ('/stylesheets/') ||
...
i18next.use (middleware.LanguageDetector).use (backend).init ({
  ...
  middleware.addRoute (i18next, '/:lng', ['en', 'he'], app, 'get', function (
    req,
    res
  ) {
    //endpoint function
...
if (process.env.DRUPAL_TICKET_SYNC_EVERY_X_MINUTES) {
  log.info (
    'Drupal ticket sync is on. Sync will run every X minutes: ',
    process.env.DRUPAL_TICKET_SYNC_EVERY_X_MINUTES
  );
  setTimeout (() => {
    var drupalTicketSync = require ('./scripts/drupal_ticket_sync');
    drupalTicketSync.runSyncTicketsLoop (
      process.env.DRUPAL_TICKET_SYNC_EVERY_X_MINUTES
    );
  }, 10000);
} else {
  log.warn (
    'Drupal ticket sync is disabled. To run, set DRUPAL_TICKET_SYNC_EVERY_X_MINUTES in your .env file'
  );
}
