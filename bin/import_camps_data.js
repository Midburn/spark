...
function main(argv) {
    ...
    Promise.all([
        ...
        //             "CREATE TRIGGER `camp_members_groups_after_upd` AFTER UPDATE ON `camp_members` FOR EACH ROW INSERT INTO users_groups_membership (group_id, user_id, status) VALUES (new.camp_id, new.user_id,new.status) ON DUPLICATE KEY UPDATE SET status = new.status  "
