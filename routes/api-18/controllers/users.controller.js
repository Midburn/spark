...
class UsersController {
  ...
  getUserById (req, res, next) {
    User.forge ({user_id: req.params.id})
      .fetch ({columns: req.query.nameOnly ? 'name' : '*'})
      .then (user => {
        if (user !== null) {
  ...
  getUserByEmail (req, res, next) {
    User.forge ({email: req.params.email})
      .fetch ()
      .then (user => {
        if (user !== null) {
  ...
  userJoinDetails (req, res) {
    if (
      req.user.isAdmin ||
      req.user.attributes.user_id === parseInt (req.params.user_id)
    ) {
      User.forge ({user_id: req.params.user_id}).fetch ().then (user => {
        ...
        user.getUserCamps (camps => {
          ...
          if (user.attributes.camp) {
            ...
            return helperService.customError (
              404,
              'Couldnt find user available camp',
              res,
              true
            );
          }
  ...
  getUsersGroups (req, res) {
    ...
    req.user.getUserCamps (
      async camps => {
        let groups = [];
        let group = {};
        let group_props;
        for (let i in camps) {
          group_props = Camp.prototype.__parsePrototype (
            camps[i].__prototype,
            req.user
          );
          if (
            ['approved', 'pending', 'pending_mgr', 'approved_mgr'].indexOf (
              camps[i].member_status
            ) > -1
          ) {
            group = {
        ...
        if (req.user.isAdmin) {
          let query =
            "SELECT camps.__prototype, SUM(IF(camp_members.status IN ('approved','approved_mgr'),1,0)) AS total FROM camps LEFT JOIN camp_members ON camps.id=camp_members.camp_id WHERE camps.event_id='" +
            (event_id || req.user.currentEventId) +
            "' GROUP BY __prototype;";
          let query1 =
            'SELECT ' +
          ...
          if (ticketStats && ticketStats[0] && ticketStats[0].length > 0) {
            ...
            stat.last_24h_first_entrance =
              ticketStats[0][0]['last_24h_first_entrance'];
            stat.last_1h_first_entrance =
              ticketStats[0][0]['last_1h_first_entrance'];
