...
class CampsService {
  ...
  updateCampStatus (current_event_id, camp_id, user_id, action, camp_mgr, res) {
    ...
    Camp.forge({id: camp_id, event_id: current_event_id})
      .fetch()
      .then(camp => {
        let camp_mgr_id;
        ...
        camp.getCampUsers(users => {
          ...
          const camp_manager = camp.isCampManager(
            camp.attributes.main_contact
          );
          // camp manager commands
          if (
            action === 'approve_new_mgr' &&
            (camp_mgr_id === camp.attributes.main_contact || isAdmin)
          ) {
            new_status = 'approved';
            if (!user) {
            ...
            if (
              user &&
              action === 'remove_mgr' &&
              user.member_status === 'approved_mgr' &&
              (camp_mgr_id === camp.attributes.main_contact || isAdmin)
            ) {
              new_status = 'approved';
            } else if (
              ...
              if (group_options.auto_approve_new_members) {
                ...
                mail_delivery.subject =
                  'Spark: you have been requested to join camp';
            ...
            if (action === 'join') {
              ...
              action === 'join_mgr' &&
              user &&
              user.member_status === 'pending_mgr' &&
              camp_mgr_id === user_id
            ) {
              new_status = 'approved';
              if (camp_manager) {
                mail_delivery.subject =
                  'Spark: A member approved himself to your camp!';
                mail_delivery.template = 'emails/camps/member_approved';
                mail_delivery.to_mail = camp_manager.email;
              }
            } else if (
              ...
              if (camp_manager) {
                mail_delivery.subject =
                  'Spark: ' + user.email + ' canceled himself from your camp!';
          ...
          const _after_update = () => {
            ...
            if (group_options.send_mail && mail_delivery.template !== '') {
              if (user) {
                let email = mail_delivery.to_mail !== ''
                  ? mail_delivery.to_mail
                  : user.email;
                helperService.emailDeliver(
                  email,
                  mail_delivery.subject,
                  mail_delivery.template,
                  {user: user, camp: camp.toJSON(), camp_manager: camp_manager}
                ); // notify the user
              } else {
          ...
          if (addinfo_jason_subAction !== null) {
            ...
            knex(constants.CAMP_MEMBERS_TABLE_NAME)
              .select(
                'user_id',
                `${constants.CAMP_MEMBERS_TABLE_NAME}.addinfo_json`,
                `${constants.EVENTS_TABLE_NAME}.addinfo_json as eventInfo`
              )
              .rightJoin(
                constants.EVENTS_TABLE_NAME,
                `${constants.EVENTS_TABLE_NAME}.event_id`,
                `${constants.EVENTS_TABLE_NAME}.event_id`
              )
              .where({
              ...
              .then(resp => {
                ...
                const allocationDates = {
                  start: isGroupSale
                    ? new Date(eventInfo.group_sale_tickets_allocation_start)
                    : new Date(
                        eventInfo.appreciation_tickets_allocation_start
                      ),
                  end: isGroupSale
                    ? new Date(eventInfo.group_sale_tickets_allocation_end)
                    : new Date(eventInfo.appreciation_tickets_allocation_end),
                };

                let jsonInfo;
                try {
                  //pass the response to the process method
                  jsonInfo = usersService.modifyUsersInfo(
                    resp[0].addinfo_json,
                    addinfo_jason_subAction,
                    camp,
                    users,
                    user,
                    isAdmin,
            ...
            if (save_method.method === 'insert') {
              query = knex(constants.CAMP_MEMBERS_TABLE_NAME)
                .insert(data)
                .toString();
            } else {
              query =
                'UPDATE ' +
                constants.CAMP_MEMBERS_TABLE_NAME +
                ' SET status="' +
                data.status +
                '" WHERE camp_id=' +
                data.camp_id +
            ...
            res.status(404).json({
              error: true,
              data: {message: 'Cannot execute this command.'},
            });
          }
        });
      })
  ...
  saveCamp (req, isNew, camp) {
    ...
    helperService.updateProp(
      data,
      req.body,
      'noise_level',
      constants.CAMP_NOISE_LEVELS
    );
    ...
    return Camp.forge({id: req.params.id})
      ...
      .then(camp => {
        return camp.save(data).then(camp => {
          if (group_props.isAdmin) {
            if (
              req.body.entrance_quota !== 'undefined' &&
              req.body.entrance_quota !== ''
            ) {
              return camp.relations.users_groups.save({
                entrance_quota: parseInt(req.body.entrance_quota),
              });
            } else return camp;
          }
        });
  ...
  prepareCampFiles (camp, user) {
    ...
    return camp.relations.files.models.map(file => {
      return {
        ...
        file_path: s3Client.getPresignedUrl(
          file.attributes.file_path,
          awsConfig.buckets.camp_file_upload
        ),
  ...
  retrieveDataFor (group_proto, user, eventId) {
    return Camp.query(query => {
      query
        ...
        .where({
          'camps.event_id': eventId || user.currentEventId,
          'camps.__prototype': group_proto,
        });
    })
      .orderBy('camp_name_en', 'ASC')
      .fetchAll({withRelated: ['members']})
      .then(dbCamps => {
        const camps = dbCamps.toJSON().map(campModel => {
          const manager = {};
